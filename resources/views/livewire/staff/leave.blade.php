...
<div class="max-w-7xl mx-auto" x-data="{
    ...
    <!-- Apply for Leave Tab -->
    <div x-show="activeTab === 'apply'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="bg-gray-100 p-2 rounded-md">
                            <span class="font-medium">{{ $calculatedDays }}
                                {{ Str::plural('day', $calculatedDays) }}</span>
                        </div>
                    </div>
                        ...
                        @foreach ($leaveRequests as $request)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClasses[$request->status] }}">
                                        {{ $statusText[$request->status] }}
                                    </span>
                                </td>
                                <!-- Add this to your leave history table, in the Actions column -->
    ...
    <!-- Leave Balance Tab -->
    <div x-show="activeTab === 'balance'" x-transition:enter="transition ease-out duration-200"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        class="bg-white shadow-md rounded-lg p-6">
    ...
    <x-modals.modal name="signature-modal" maxWidth="md">
        <div x-data="signaturePad()">
            ...
            <button x-on:click="upload"
                class="text-black bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mr-2" viewBox="0 0 20 20"
                    fill="currentColor" aria-hidden="true">
                    <path
                        d="M10 1a9 9 0 100 18 9 9 0 000-18zm1 14.414V11h3.586l-4.293-4.293L8.414 7.586l4.293 4.293H11v4.414z" />
                </svg>
                Submit
            </button>
        </div>
                        ...
                        if (event.detail === 'signature-modal') {
                            setTimeout(() => {
                                if (this.signaturePadInstance) {
                                    ...
                                    this.signaturePadInstance = new SignaturePad(this
                                        .$refs.signature_canvas);
                                }
                            }, 100);
                        }
                    });
